...
namespace PolarPet
{
    ...
    public sealed class SoapItem : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
    {
        ...
        [Header("AI Integrate")]
        [Tooltip("可選：搓澡期間暫停 PetAIChatController（enabled=false），結束後恢復。")]
        [SerializeField] private global::PetAIChatController petAIChatController;
        [Tooltip("可選：若寵物上有 DragPolar，搓澡期間會 BeginInteraction 以暫停自動移動。")]
        [SerializeField] private global::DragPolar dragPolar;
        ...
        private void Awake()
        {
            ...
            if (petAIChatController == null)
            {
                petAIChatController = FindObjectOfType<global::PetAIChatController>();
            }
        }
        ...
        private void StartBathing(Animator petAnimator)
        {
            ...
            SetPetAIChatEnabled(false);
            SetPetMovementLocked(true);
        ...
        private void StopBathing()
        {
            ...
            SetPetAIChatEnabled(true);
            SetPetMovementLocked(false);

            currentPetAnimator = null;
        }

        private void SetPetAIChatEnabled(bool enabled)
        {
            if (petAIChatController == null)
            {
                petAIChatController = FindPetAIChatControllerOnPet();
            }

            if (petAIChatController == null) return;
            petAIChatController.enabled = enabled;
        }

        private global::PetAIChatController FindPetAIChatControllerOnPet()
        {
            if (currentPetAnimator == null) return null;
            return currentPetAnimator.GetComponentInParent<global::PetAIChatController>(true);
        }

        private void SetPetMovementLocked(bool locked)
        {
            if (dragPolar == null)
            {
                dragPolar = FindDragPolarOnPet();
            }

            if (dragPolar == null) return;

            if (locked)
            {
                dragPolar.BeginInteraction();
            }
            else
            {
                dragPolar.EndInteraction();
            }
        }

        private global::DragPolar FindDragPolarOnPet()
        {
            if (currentPetAnimator == null) return null;
            return currentPetAnimator.GetComponentInParent<global::DragPolar>(true);
        }
